                    ...
                    sapi.listInstances(
                        ...
                        function onInsts(err, insts) {
                            if (err) {
                                ...
                                // *all* DCs in this region. We need to pair
                                // down to just those in this DC. We'll rely
                                // on `metadata.DATACENTER` for each SAPI obj.
                                storInsts = insts.filter(
                                    function inThisDc(inst) {
                                        assert.string(inst.metadata.DATACENTER,
                                            'inst.metadata.DATACENTER');
                                        return (inst.metadata.DATACENTER
...
// or image UUID) in the 'dev' channel of updates.joyent.com.
//
