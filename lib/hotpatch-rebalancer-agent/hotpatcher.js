    ...
    sapi.listServices(
        ...
        function onSvcs(err, svcs) {
            ...
            if (!svcs || svcs.length < 1) {
                ...
                assert(
                    svcs.length === 1,
                    'multiple "' + svcName + '" services were found: '  +
                        JSON.stringify(svcs)
                );
                svc = svcs[0];
            }
...
`;
    ...
    storInsts.forEach(inst => storInstFromUuid[inst.uuid] = inst);
    ...
    exec.on('data', function onData(res) {
        ...
        if (res.result.exit_status !== 0) {
            execErrs.push(new VError({info: {res: res}},
            ...
            if (hotpatchedStr.startsWith('error:')) {
                execErrs.push(new VError({info: {res: res, hotpatchedStr: hotpatchedStr}},
                    'error getting rebalancer-agent install status on storage inst ' +
                    res.zonename));
            } else if (versionStr.startsWith('error:')) {
                execErrs.push(new VError({info: {res: res, versionStr: versionStr}},
