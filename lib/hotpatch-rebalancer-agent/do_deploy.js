    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                // Before the following commit to "manta-mako.git":
                //
                //    commit 555e7ac5e8ce9d72ef9399adbccc693cd57379fb
                //    Date:   2020-04-30T17:11:05+01:00
                //
                //        TOOLS-2461 upgrade Manta/Triton 19.[12] components to 19.4 (#31)
                //
                // mantav2-storage images were built on an origin (base) image
                // earlier than "triton-origin-x86_64-19.4.0". All current
                // "mantav2-rebalancer-agent" images we are considering are
                // built for an only work on a storage instance using the
                // new 19.4.0 origin image. This translates to "mantav2-storage"
                // images after (and including):
                //
                //      master-20200430T161240Z-g555e7ac
                //
                // We check that here.
                function validateSufficientStorImages(ctx, next) {
                    const MIN_STOR_BUILDTIME = '20200430T161240Z';
                    const STOR_VERSION_RE = /^([\w-]+)-(\d{8}T\d{6}Z)-(g[0-9a-f]{7})$/;

                    hotpatcher.fetchStorInstImages(ctx.storInsts, function(
                        err
                    ) {
                        if (err) {
                            next(err);
                        ...
                        // Determine if any storage insts are too old.
                        var errs = [];
                        for (let storInst of ctx.storInsts) {
                            if (!storInst.image) {
                                ui.error(
                                    'warn: cannot verify that storage ' +
                                        'inst %s includes TOOLS-2461, necessary ' +
                                        'for rebalancer-agent hotpatches: its ' +
                                        'image is not in IMGAPI',
                                    storInst.uuid
                                );
                            } else {
                                let match = STOR_VERSION_RE.exec(
                                    storInst.image.version
                                );
                                if (!match) {
                                    ui.error(
                                        'warn: cannot verify that storage ' +
                                            'inst %s includes TOOLS-2461, necessary ' +
                                            'for rebalancer-agent hotpatches: could ' +
                                            'not parse its image version "%s"',
                                        storInst.uuid,
                                        storInst.image.version
                                    );
                                } else if (match[2] < MIN_STOR_BUILDTIME) {
                                    errs.push(
                                        new VError(
                                            'stor inst %s is too old for ' +
                                                'rebalancer-agent hotpatches: must ' +
                                                'be at least %s to include TOOLS-2461',
                                            storInst.uuid,
                                            MIN_STOR_BUILDTIME
                                        )
                                    );
                    ...
                    hotpatcher.getRemoteImage(imageUuid, function onImg(
                        ...
                        if (err) {
                            next(
                                new VError(
                                    err,
                                    'error getting image %s from updates.tritondatacenter.com (dev channel)',
                                    imageUuid
                                )
                            );
                        } else if (img.name !== 'mantav2-rebalancer-agent') {
                ...
                function confirm(ctx, next) {
                    ...
                    if (!ctx.localImg) {
                        ui.infoBullet(
                            'Import rebalancer-agent image %s (%s) from updates.tritondatacenter.com.',
                            ctx.remoteImg.uuid,
                ...
                function importImageIfNecessary(ctx, next) {
                    ...
                    ui.info(
                        'Importing image %s from updates.tritondatacenter.com',
                        ctx.remoteImg.uuid
                    );
                        ...
                        function onImported(err, img) {
                            if (err) {
                                next(
                                    new VError(
                                        err,
                                        'could not import image %s from updates.tritondatacenter.com',
                                        ctx.remoteImg.uuid
                                    )
                                );
                            } else {
