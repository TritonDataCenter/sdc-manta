...
/*
 * Storage and minio zone deployments cannot be done concurrently, so we funnel
 * those zone deployments through a vasync Queue with concurrency 1.  This is
...
var dMinioQueue = vasync.queue(function(func, callback) {
    func(callback);
}, 1);
...
function createInstance(deployer, app, svc, cb) {
    ...
    if (deployer.svcname === 'minio') {
        metadata.SERVICE_NAME = sprintf('minio.%s', service_root);
    }
    ...
    async.waterfall(
        [
            function(subcb) {
                if (svc.name !== 'storage' && svc.name !== 'minio') {
                    subcb(null);
                    return;
                }

                log.debug(
                    'putting "%s" zone provision for ' + '"%s" into the queue',
                    svc.name,
                    inst_uuid
                );
                var dServiceQueue;
                if (svc.name === 'storage') {
                    dServiceQueue = dStorageQueue;
                } else if (svc.name === 'minio') {
                    dServiceQueue = dMinioQueue;
                }
                dServiceQueue.push(function(_queuecb) {
                    /*
                     * When we reach here, we're the only "storage" or "minio"
                     ...
                     */
                    if (svc.name === 'storage') {
                        storageQueuecb = _queuecb;
                    } else if (svc.name === 'minio') {
                        minioQueuecb = _queuecb;
                    }
                    log.debug(
                        'dequeueing "%s" zone provision for "%s"',
                        svc.name,
                        inst_uuid
                    );
                    subcb();
            ...
            function(subcb) {
                ...
                /*
                 * The manta_[storage|minio]_id should be the next available
                 * number.
                 */
                var opts = {};
                ...
                log.info('finding next manta_%s_id', svc.name);

                sapi.listInstances(opts, function(err, insts) {
                    if (err) {
                        log.error(err, 'failed to list %s instances', svc.name);
                        subcb(err);
                        return;
                    }

                    if (svc.name === 'storage') {
                        var mStorageFlavorId = pickNextStorageFlavorId(
                            insts,
                            metadata.SERVICE_NAME,
                            svc.name,
                            null
                        );
                        if (mStorageFlavorId instanceof Error) {
                            subcb(mStorageFlavorId);
                            return;
                        }
                        metadata.MANTA_STORAGE_ID = mStorageFlavorId;
                        params.tags.manta_storage_id = mStorageFlavorId;
                        subcb();
                    } else if (svc.name === 'minio') {
                        // Work out which rack this minio instance is being
                        // deployed to so that we can add it to the minio_id.
                        // We'll save the rack name as an additional bit of
                        // metadata in case it's useful.
                        var rackName;
                        common.getServerNicTags.call(deployer, function(
                            tags_err,
                            tags
                        ) {
                            if (tags_err !== null) {
                                log.error(
                                    'We had a problem obtaining nic tags!',
                                    tags_err
                                );
                                subcb(tags_err);
                                return;
                            }
                            // XXX timf: I don't know if this is the correct
                            // way to determine a rack id. If it is, perhaps
                            // we should include it in triton-netconfig.git?
                            // Find the name of the manta rack, if any.
                            for (var i = 0; i < tags.online.length; i++) {
                                if (tags.online[i].startsWith('manta')) {
                                    continue;
                                }
                                rackName = tags.online[i].replace(
                                    'manta_rack_',
                                    ''
                                );
                                if (rackName === tags.online[i]) {
                                    rackName = null;
                                }
                                // XXX TIMF HARDCODE RACK NAME HERE!
                                rackName = 'noodles';
                            }
                            var minioStorageFlavorId = pickNextStorageFlavorId(
                                insts,
                                metadata.SERVICE_NAME,
                                svc.name,
                                rackName
                            );
                            if (minioStorageFlavorId instanceof Error) {
                                subcb(minioStorageFlavorId);
                                return;
                            }
                            metadata.MANTA_MINIO_ID = minioStorageFlavorId;
                            if (rackName !== null) {
                                metadata.MANTA_RACK = rackName;
                            }
                            params.tags.manta_minio_id = minioStorageFlavorId;
                            subcb();
                        });
                    }
                });
        ...
        function() {
            if (storageQueuecb) {
                ...
                setTimeout(storageQueuecb, 0);
            } else if (minioQueuecb) {
                log.debug(
                    'done with "minio" zone provision for "%s"',
                    inst_uuid
                );
                setTimeout(minioQueuecb, 0);
            }
...
/*
 * Given a list of SAPI instances for storage or minio nodes, return an unused
 * Manta storage or minio id.  If we're at all unsure, we return an error rather
 * than potentially returning a conflicting name.
 */
function pickNextStorageFlavorId(instances, svcname, manta_role, rack) {
    var max, inst, instname, numpart, suffix;
    var i, p, n;
    var err = null;

    if (manta_role === 'minio') {
        if (rack !== null) {
            // for rack-aware networks, include the rack name in the minio_id
            suffix = '.' + rack + '.' + svcname;
        } else {
            suffix = '.' + svcname;
        }
    ...
    for (i = 0; i < instances.length; i++) {
        inst = instances[i];
        // Only look at instances of the same role
        if (inst.params.tags.manta_role !== manta_role) {
            continue;
        }
        if (manta_role === 'storage') {
            instname = inst.metadata.MANTA_STORAGE_ID;
        } else if (manta_role === 'minio') {
            instname = inst.metadata.MANTA_MINIO_ID;
        } else {
            err = new VError('invalid service for pickNextStorageFlavorId');
            break;
        }
        ...
        if (typeof instname !== 'string') {
            err = new VError(
                'instance "%s": missing or ' +
                    'invalid MANTA_STORAGE_ID or MANTA_MINIO_ID metadata',
                inst.uuid
            );
            break;
        }

        p = instname.indexOf(suffix);
        if (p === -1 || p === 0) {
            if (manta_role === 'minio') {
                // We can have multiple different racks for minio. Just ignore
                // any minio instances that aren't in the expected rack.
                continue;
            } else {
                err = new VError(
    ...
    if (err !== null) {
        return new VError(err, 'failed to allocate MANTA_STORAGE|MINIO_ID');
    }

    return sprintf('%d%s', max + 1, suffix);
}
