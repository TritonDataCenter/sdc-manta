    ...
    async.waterfall(
        [
            ...
            function checkShardConfigs(cb) {
                ...
                if (
                    typeof md[common.MARLIN_SHARD] !== 'string' ||
    ...
    async.waterfall(
        [
            ...
            function updateZKServers(nic, cb) {
                ...
                sapi.updateApplication(self.manta_app.uuid, opts, function(
                    err,
                    app
                ) {
                    if (!err) {
            ...
            function ensureComputeId(cb) {
                ...
                common.getOrCreateComputeId.call(self, serverUuid, function(
                    err,
                    cid
                ) {
                    if (err) {
            ...
            function configureMarlinComputeZone(cb) {
                ...
                log.info('configuring compute zone, ' + 'getting vmapi object');
                vmapi.getVm(params, function(err, vm) {
                    ...
                    cnapi.commandExecute(server, script, function(suberr) {
                        if (suberr) {
                            log.error(
                                suberr,
                                'failed to ' + 'configure compute zone %s',
                                params.uuid
                            );
                            return cb(err);
                        }

                        log.info('configured compute zone %s', params.uuid);
                        return cb(null);
                    });
                });
            },
    ...
    async.waterfall(
        [
            ...
            function rmMarlinZone(cb) {
                ...
                var log = self.log;
                var scriptpath = sprintf(
                    '%s/tools/mrzoneremove %s',
                    common.MARLIN_DIR,
                    instance
                );
                log.info(
                ...
                self.CNAPI.commandExecute(cnid, scriptpath, function(err) {
                    if (err) {
                        err = new VError(
                            err,
                            'failed to remove compute zone ' + '"%s"',
                            instance
                        );
                        log.error(err);
                        cb(err);
    ...
    async.waterfall(
        [
            function getNicTags(subcb) {
                common.getServerNicTags.call(self, function(err, tags) {
                    if (err) {
                        subcb(err);
                        return;
                    }
...
function createInstance(self, app, svc, cb) {
    ...
    assert.string(self.config.datacenter_name, 'self.config.datacenter_name');
    ...
    if (self.svcname === 'buckets-postgres') {
        metadata.SERVICE_NAME = sprintf('%s.boray.%s', shard, service_root);
        metadata.MANATEE_SHARD_PATH = sprintf(
            '/manatee/%s',
            metadata.SERVICE_NAME
        );
