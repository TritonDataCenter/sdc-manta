 ...
 * lib/layout.js.
 *
 * Note: `manta-adm update ...` deployments will (re)provision service instances
 * one service at a time, in the order specified in this array. As a result
 * some orderings here are important.
 */
var mSvcNames = [
    // nameservice must be deployed first, because almost all services depend
    // on it for name resolution, and many for its ZK cluster.
    'nameservice',
    'postgres',
    'moray',
    'electric-moray',
    'storage',
    'authcache',
    // webapi must come after moray, because moray initial deployment setup.sh
    // creates the database and user, then webapi initial deployment setup.sh
    // does the rest.
    'webapi',
    'loadbalancer',
    'ops',
    ...
    'buckets-postgres',
    'buckets-mdplacement',
    // buckets-mdapi must be after buckets-mdplacement because its setup.sh
    // assumes buckets-mdplacement is running.
    'buckets-mdapi',
    // mantad is the next-generation S3 substrate (single Rust binary
    // running on a delegated dataset). Self-contained: depends on
    // nothing earlier in this list, so its position here is
    // independent.
    'mantad'
];
...
var mSvcConfigs = {
    ...
    'buckets-mdapi': {
        sharded: true,
        imageNames: ['mantav2-buckets-mdapi', 'mantav2-boray', 'manta-boray']
    },
    'buckets-mdplacement': {
        sharded: false,
        imageNames: [
            'mantav2-buckets-mdplacement',
            'mantav2-electric-boray',
            'manta-electric-boray'
        ]
    },
    mantad: {
        sharded: false,
        imageNames: ['mantav2-mantad']
    }
};
