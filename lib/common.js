...
exports.BUCKETS_MDAPI_ENDPOINTS = 'BUCKETS_MDAPI_ENDPOINTS';
...
function findServerUuid(cb) {
    ...
    cnapi.listServers(function(err, servers) {
        ...
        if (servers.length !== 1) {
            message =
                'findServerUuid failed, invalid number of ' +
                'servers in cnapi';
            e = new Error(message);
            e.message = message;
            log.error(
...
function cid_getServerComputeIdMapping(cid, next) {
    ...
    getMantaApplication.call(cid.cid_self, function(err, ao) {
        if (err) {
            next(new VError(err, 'could not load manta application from SAPI'));
            return;
        }
...
function cid_ensureMappingExists(cid, next) {
    ...
    for (var i = 0; i < servers.length; i++) {
        ...
        if (seen[compute_id] !== undefined) {
            ...
            next(
                new VError(
                    'multiple servers found with compute ' +
                        'ID "%s": "%s" and "%s"',
                    compute_id,
                    uuid,
                    seen[compute_id]
        ...
        if (parsed === null || parsed.domain !== domain) {
            next(
                new VError(
                    'found invalid compute ID mapping ' +
                        'for server "%s": "%s"',
                    uuid,
                    compute_id
                )
...
function cid_flushToSAPI(cid, next) {
    ...
    sapi.updateApplication(app.uuid, update, function(err) {
        if (err) {
            next(new VError(err, 'failed to update manta application in SAPI'));
            return;
        }
        ...
        vasync.forEachParallel(
            {
                func: function(uuid, subcb) {
                    commandExecute.call(self, uuid, script, subcb);
                },
                inputs: uuids
            },
            cb
...
function commandExecute(server_uuid, script, cb) {
    ...
    cnapi.commandExecute(server_uuid, script, function(err) {
        if (err) {
            log.error(err, 'failed to execute command on %s', server_uuid);
            return cb(err);
        }
...
function updateNetworkUsers(opts, callback) {
    ...
    log.info(
        'Attempting to %s user %s permission\'s for network "%s"',
        action,
        owner_uuid,
        name
    );
    ...
    function _updateNetworkUserCommon(network, cb) {
        ...
        update_func(owner_uuid, net_owner_uuids, function(err, uuids) {
            if (err) {
                ...
                log.warn(
                    {err: err},
                    'could not update user %s on network "%s"',
                    owner_uuid,
                    name
                );
                if (
                    (action === 'add' && err.name === 'UserExistsError') ||
                    (action === 'remove' && err.name === 'UserNotFoundError')
                ) {
                    e = null;
                }
            ...
            napi.updateNetwork(network.uuid, {owner_uuids: uuids}, function(
                suberr
            ) {
                if (suberr) {
                ...
                log.info('successfully updated network "%s"', name);
                cb(null);
                return;
            });
        });
                ...
                function updatePools(n, cb) {
                    ...
                    napi.listNetworkPools({name: n}, function(err, pools) {
                        if (err) {
                            log.error(
                                err,
                                'failed to list network pools of name ' +
                                    '"%s"',
                                n
                            );
                            cb(err);
                        ...
                        if (pools.length > 1) {
                            log.warn(
                                'more than one network pool with the name ' +
                                    '"%s" found, skipping all',
                                n
                            );
                            cb(null);
                            return;
