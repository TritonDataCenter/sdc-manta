...
MantaAmonUpdatePlan.prototype.needsChanges = function() {
    return (
        this.mup_groups_remove.length > 0 ||
...
MantaAmonUpdatePlan.prototype.probeRemove = function probeRemove(probe) {
    this.probeUpdate(probe, this.mup_nremove_bygroup, this.mup_probes_remove);
};
...
function amonUpdatePlanCreate(args) {
    ...
    if (wanted instanceof Error) {
        return new VError(wanted, 'generating wanted amon configuration');
    }
     ...
     */
    wanted.eachOrphanProbe(function(_probe) {
        throw new VError('unexpected orphan probe in "wanted" set');
    });
    ...
    wanted.eachProbeGroup(function iterWProbeGroup(wpg) {
        ...
        wanted.eachProbeGroupProbe(pgname, function iterWProbe(wp) {
            ...
            for (i = 0; i < dprobes.length; i++) {
                ...
                if (
                    dp.p_type === wp.p_type &&
                    jsprim.deepEqual(dp.p_config, wp.p_config) &&
            ...
            rv.mup_nprobesmatch++;
            if (
                wp.p_group_events !== dp.p_group_events ||
                (dp.p_contacts === null && wp.p_contacts !== null) ||
                (dp.p_contacts !== null && wp.p_contacts === null) ||
                (dp.p_contacts !== null &&
                    !jsprim.deepEqual(
                        dp.p_contacts.slice(0).sort(),
         ...
         * should be non-empty.
         */
        jsprim.forEachKey(probesByAgent, function(_agent, dprobes) {
    ...
    deployed.eachOrphanProbe(function(_probe) {
...
function amonGenerateWantedTemplate(args) {
    ...
    if (pt.pt_scope.ptsc_service === 'each') {
        ...
        pt.pt_aliases.forEach(function(alias) {
            ...
            eventForSvc[alias.pta_service] = alias.pta_event;
        });
    ...
    events.forEach(function(eventName) {
        var pgname, error;
        pgname = alarm_metadata.probeGroupNameForTemplate(pt, eventName);
    ...
    jsprim.forEachKey(eventForSvc, function(svcname, eventName) {
        ...
        if (pt.pt_scope.ptsc_global) {
            ...
            checkers.forEach(function(c) {
                ...
                assertplus.string(instances[c].inst_server_uuid);
        ...
        if (pt.pt_scope.ptsc_check_from !== null) {
            if (!instancesBySvc.hasOwnProperty(pt.pt_scope.ptsc_check_from)) {
                return;
            }
        ...
        probeargs.forEach(function(p) {
            pt.pt_checks.forEach(function(check, i) {
                ...
                probe = {
                    ...
                    agent: p.agent,
                    machine: p.machine,
                    group: alarm_metadata.probeGroupNameForTemplate(
                        pt,
                        eventName
                    ),
                    groupEvents: true
                 ...
                 */
                label = sprintf(
                    'probe for group "%s", check %d, machine "%s"',
                    probe.group,
                    i + 1,
                    p.machine
                );
                md = instances.hasOwnProperty(p.target)
                    ? instances[p.target].inst_metadata
                    : null;
                amonProbePopulateAutoEnv(label, probe, md, errors);
...
function amonProbePopulateAutoEnv(label, probe, metadata, errors) {
    ...
    if (metadata === null) {
        errors.push(
            new VError(
                '%s: "autoEnv" specified but no ' +
                    'metadata found for instance',
                label
            )
        );
        return;
...
function amonUpdatePlanCreateUnconfigure(args) {
    ...
    deployed.eachProbeGroup(function iterDProbeGroup(dpg) {
        ...
        if (wpg === null && !metadata.probeGroupIsRemovable(pgname)) {
...
function amonUpdatePlanSummarize(args) {
    ...
    if (plan.mup_groups_remove.length === 0) {
        ...
        plan.mup_groups_remove.forEach(function(pg) {
            assertplus.ok(!plan.mup_nadd_bygroup.hasOwnProperty(pg.pg_uuid));
            nprobes = plan.mup_nremove_bygroup.hasOwnProperty(pg.pg_uuid)
                ? plan.mup_nremove_bygroup[pg.pg_uuid]
                : 0;

            nagents = plan.mup_agents_bygroup.hasOwnProperty(pg.pg_uuid)
                ? Object.keys(plan.mup_agents_bygroup[pg.pg_uuid]).length
                : 0;
    ...
    if (plan.mup_groups_add.length === 0) {
        ...
        plan.mup_groups_add.forEach(function(pg) {
            ...
            assertplus.ok(!plan.mup_nremove_bygroup.hasOwnProperty(pg.pg_uuid));
            ...
            if (plan.mup_nadd_bygroup.hasOwnProperty(pg.pg_uuid)) {
                ...
                assertplus.ok(
                    plan.mup_agents_bygroup.hasOwnProperty(pg.pg_uuid)
                );
                nagents = Object.keys(plan.mup_agents_bygroup[pg.pg_uuid])
                    .length;
            } else {
                assertplus.ok(
                    !plan.mup_agents_bygroup.hasOwnProperty(pg.pg_uuid)
    ...
    if (verbose) {
        ...
        probes.forEach(function(p) {
            fprintf(
                ...
                '    %s %-16s %s\n',
                p.p_agent,
                instances[p.p_agent].inst_svcname,
                p.p_name
            );
        });
        fprintf(out, '\n');
    ...
    fprintf(
        out,
        '%6d wanted probe groups matched existing groups\n',
        plan.mup_ngroupsmatch
    );
    fprintf(
        out,
        '%6d wanted probes matched existing probes\n',
        plan.mup_nprobesmatch
    );
    fprintf(
        out,
        '%6d probes ignored because they were in no probe group\n',
        plan.mup_nprobesorphan
    );
    fprintf(
        out,
        '%6d probe groups ignored (operator-added)\n',
        plan.mup_ngroupsignore
    );
    fprintf(
        out,
        '%6d total probe groups to remove\n',
...
function amonUpdatePlanApply(args, callback) {
     ...
     */
    jsprim.forEachKey(
        au.mau_plan.mup_deployed.mac_probegroups_by_name,
    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                function amonUpdateRemoveProbes(_, subcallback) {
                    amonUpdateQueue(
                        au,
                        plan.mup_probes_remove,
                        amonUpdateProbeRemove,
                        subcallback
                    );
                },
                function amonUpdateRemoveProbeGroups(_, subcallback) {
                    amonUpdateQueue(
                        au,
                        plan.mup_groups_remove,
                        amonUpdateGroupRemove,
                        subcallback
                    );
                },
...
function amonUpdateProbeAdd(au, probe, callback) {
    ...
    au.mau_amon.createProbe(au.mau_account, newprobe, function onAmonProbeAdd(
        err
    ) {
        if (err) {
...
function amonUpdateProbeRemove(au, probe, callback) {
    ...
    au.mau_amon.deleteProbe(
        au.mau_account,
        probe.p_uuid,
        function onAmonProbeRemove(err) {
            if (err) {
                err = new VError(err, 'remove probe "%s"', probe.p_uuid);
            } else {
                au.mau_nprobes_removed++;
            }
...
function amonUpdateGroupAdd(au, group, callback) {
    ...
    au.mau_amon.createProbeGroup(
        au.mau_account,
        newgroup,
        function onAmonGroupAdd(err, amongroup) {
            if (!err && typeof amongroup.uuid !== 'string') {
                err = new VError(
                    'amon returned group with bad or missing uuid'
                );
            }

            if (!err && amongroup.name !== group.pg_name) {
                err = new VError(
                    'amon returned group with a ' +
                        'different name (uuid "%s")',
                    amongroup.uuid
                );
            }
...
function amonUpdateGroupRemove(au, group, callback) {
    ...
    au.mau_amon.deleteProbeGroup(
        au.mau_account,
        group.pg_uuid,
        function onAmonGroupRemove(err) {
            if (err) {
                err = new VError(err, 'remove group "%s"', group.pg_uuid);
            } else {
                au.mau_ngroups_removed++;
            }
