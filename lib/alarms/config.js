    ...
    amon.listProbeGroups(account, function(err, rawgroups) {
        ...
        rawgroups.forEach(function(rawgroup) {
            ...
            if (error instanceof Error) {
                errors.push(new VError(error, 'ignoring group'));
            }
        });
...
function amonLoadComponentProbes(args, callback) {
            ...
            amonFetchAgentProbes(
                ...
                function(err, probes) {
                    if (err) {
                        err = new VError(
                            err,
                            'fetching probes for agent on %s "%s"',
                            component.type,
                            component.uuid
                        );
                        errors.push(err);
                    ...
                    probes.forEach(function(p) {
                        ...
                        if (error !== null) {
                            warnings.push(new VError(error, 'ignoring probe'));
                        }
                    });
    ...
    queue.on('end', function() {
        ...
        callback(VError.errorFromList(errors), VError.errorFromList(warnings));
    });
...
function amonFetchAgentProbes(args, callback) {
    ...
    client.get(uripath, function(err, req, res, result) {
        ...
        if (!err && result.length === limit) {
            err = new VError('got %d results, assuming truncation', limit);
        }
 ...
 */
function amonConfigSummarize(args) {
    ...
    config.eachProbeGroup(function(pg) {
        ...
        config.eachProbeGroupProbe(pg.pg_name, function iterProbe(p) {
            ...
            ngroupprobes++;
            svcname =
                instanceSvcname.hasOwnProperty(p.p_agent) &&
                svcs.hasOwnProperty(instanceSvcname[p.p_agent])
                    ? instanceSvcname[p.p_agent]
                    : 'unknown';
            svc = svcs[svcname];
            svc.svc_nprobes++;
    ...
    Object.keys(svcs)
        ...
        .forEach(function(svcname) {
            ...
            fprintf(
                ...
                '    %-16s  %7d  %7d  %7d  %8d\n',
                svcname,
                Object.keys(svc.svc_groups).length,
                nsvcagents,
                svc.svc_nprobes,
                svc.svc_norphans
            );
...
MantaAmonConfig.prototype.addProbeGroup = function(groupdef) {
    ...
    if (this.mac_probegroups_by_uuid.hasOwnProperty(probegroup.pg_uuid)) {
        return new VError(
            'duplicate probe group uuid: "%s"',
            probegroup.pg_uuid
        );
    }

    assertplus.ok(
        !this.mac_probes_by_probegroup.hasOwnProperty(probegroup.pg_name)
    );
...
MantaAmonConfig.prototype.eachProbeGroupProbe = function(pgname, func) {
    ...
    assertplus.func(func, 'func');
    assertplus.ok(
        this.mac_probes_by_probegroup.hasOwnProperty(pgname),
...
MantaAmonConfig.prototype.eachOrphanProbe = function(func) {
    assertplus.func(func, 'func');
    this.mac_probes_orphan.forEach(function(p) {
        func(p);
    });
};
