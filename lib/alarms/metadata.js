 ...
 *
 *                                  "critical"  the data path may be
 *                                              significantly affected or may be
 *                                              imminently so.  "Affected" here
 *                                              means increased error rate or
 *                                              latency for end user operations.
 *
 *                                  "major"     the data path may be affected,
 *                                              but the impact is likely minor
 *                                              or limited to a small number of
 *                                              requests.  Even if not currently
 *                                              affected, these paths are at
 *                                              risk for a major disruption.
 *
 *                                  "minor"     the data path is likely not
 *                                              currently affected (not more
 *                                              than a bounded, fixed number of
 *                                              requests).
 *
 ...
 * See above for allowed callers.
 */
MantaAmonMetadata.prototype.probeGroupEventName = function probeGroupEventName(
    probeGroupName
) {
    var result;
 ...
 * See above for allowed callers.
 */
MantaAmonMetadata.prototype.probeGroupIsRemovable = function probeGroupIsRemovable(
    probeGroupName
) {
...
MantaAmonMetadata.prototype.addTemplate = function addTemplate(args) {
    ...
    if (this.mam_templates_byevent.hasOwnProperty(eventName)) {
        return new VError(
            '%s: re-uses event name "%s" previously used in template "%s"',
            args.originLabel,
            eventName,
    ...
    if (pt.pt_scope.ptsc_service !== 'each') {
        if (/[^a-zA-Z0-9_.]/.test(eventName)) {
            return new VError(
                '%s: event name contains unsupported characters',
                args.originLabel
            );
        }
    ...
    if (error === null && nsubs === 0) {
        return new VError(
            'template "%s": templates with scope ' +
    ...
    pt.pt_aliases.forEach(function(alias) {
        assertplus.ok(
            !self.mam_event_aliases.hasOwnProperty(alias.pta_event),
            'duplicate alias: ' + alias.pta_event
        );
...
MantaAmonMetadata.prototype.resolveEventName = function(eventName) {
    if (this.mam_event_aliases.hasOwnProperty(eventName)) {
        assertplus.ok(
            this.mam_templates_byevent.hasOwnProperty(
            ...
            scope: {
                ...
                properties: {
                    service: {
                        type: 'string',
                        required: true,
                        enum: ['each', 'all'].concat(services.mSvcNames)
                    },
                    global: {
                        type: 'boolean'
                    },
 ...
 */
MetadataLoader.prototype.loadFromDirectory = function loadFromDirectory(
    directory,
    callback
) {
    var files;
    ...
    this.mdl_load_pipeline = vasync.pipeline(
        {
            funcs: [
                function listDirectory(_, subcallback) {
                    fs.readdir(directory, function onReaddirDone(err, entries) {
                        if (err) {
                            err = new VError(err, 'readdir "%s"', directory);
                ...
                function readFiles(_, subcallback) {
 ...
 */
MetadataLoader.prototype.loadFromFile = function loadFromFile(
    filename,
    callback
) {
    var self = this;
...
MetadataLoader.prototype.loadFromString = function loadFromString(
    contents,
    inputlabel
) {
    var parsed, err;
    ...
    parsed.forEach(function(p, i) {
        ...
        for (k in p.ka) {
            if (jsprim.endsWith(p.ka[k], '\n')) {
                error = new VError(
                    '%s: field ka.%s: ends with trailing newline',
                    label,
                    k
                );
                break;
...
function parseProbeGroupName(probeGroupName) {
    ...
    if (verpart !== '1') {
        result.error = new VError(
            'unrecognized version "%s" in probe group with name "%s"',
            verpart,
            probeGroupName
        );
        return result;
