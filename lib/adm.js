...
maAdm.prototype.dumpConfigCommon = function(options, conf, callback) {
    ...
    self.ma_sdc.CNAPI.listServers({headnode: true}, function(err, servers) {
        ...
        if (
            ...
            err = new VError(
                'expected array of exactly 1 ' + 'headnode server'
            );
        }
...
maAdm.prototype.enableSnaplinks = function(args, callback) {
    ...
    self.ma_sdc.UFDS.getUser(account, function(err, user) {
        ...
        if (!found) {
            callback(
                new VError(
                    'snaplinks already enabled for ' + 'account ' + account
                )
            );
            return;
        }
        ...
        function(err) {
            if (err) {
                callback(
                    new VError(
                        err,
                        'error listing snaplink ' + 'disabled accounts'
                    )
                );
                return;
            }
...
maAdm.prototype.dumpZkServers = function(sout, conf) {
    ...
    fixable = zkconfig.missingInstances.map(function(i) {
        return new VError(
            'ZK_SERVERS[%s] has no ' + 'associated SAPI instance',
            i
        );
    });
...
maAdm.prototype.readConfigRaw = function(contents) {
    ...
    if (cnids.indexOf('<any>') != -1 && cnids.length > 1) {
        return new VError(
            'cannot combine "<any>" with ' + 'specific compute nodes'
        );
    }
