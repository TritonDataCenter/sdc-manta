...
maAdm.prototype.getDeployedServiceMetadata =
	...
	fetchImages(fetch_images_opts,
		...
		checkUuids.forEach(function checkUuid(uuid) {
			...
			if (remote_uuids.indexOf(uuid) === -1) {
				...
				if (imgNames.indexOf(
				    remote_images[uuid].name) === -1) {
					var rmsg = sprintf(
					    '%s (%s, not one of %s)', uuid,
		...
		if (imagesNotInTheChannel.length > 0) {
			if (!self.ma_channel) {
				...
				msgs.push(sprintf(
				    ...
				    'provisioned.\n' +
				    '(Use --skip-verify-channel to ' +
				    'override or pass a different channel ' +
				    'via the -C argument):\n    %s',
				    self.ma_channel,
				    imagesNotInTheChannel.join('\n    ')));
			}
