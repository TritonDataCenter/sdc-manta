...
echo "Completed stordelink successfully." >&2
`;
...
echo "Completed moraydelink successfully." >&2
`;
...
Migrator.prototype.deleteMantaMetadata = function deleteMantaMetadata(names, cb) {
...
Migrator.prototype.ensureDelinkScripts = function ensureDelinkScripts(
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            if (obj.size === 0) {
                ...
                for (let s = 0; s < obj.storageIds.length; s++) {
                    ...
                    //          ln $old $new
                    //      fi
                    let oldPath = `/manta/${obj.creatorId}/${obj.objectId}`;
                    let newPath = `/manta/${obj.creatorId}/${newUuid}`;
    ...
    for (let i = 0; i < storIds.length; i++) {
        ...
        let script = STORDELINK_TEMPLATE
            .replace('{{cmds}}', cmds.join('\n'))
            .replace('{{storage_id}}', storId);
        try {
    ...
    for (let i = 0; i < shardHosts.length; i++) {
        ...
        let filename = path.join(
            delinkDir,
            `${shardHost}_moraydelink.sh`
        );
        let script = MORAYDELINK_TEMPLATE
            .replace('{{cmds}}', cmds.join('\n'))
            .replace('{{shardHost}}', shardHost);
        try {
            fs.writeFileSync(filename, script, {encoding: 'utf8'});
        } catch (writeErr) {
            cb(writeErr);
