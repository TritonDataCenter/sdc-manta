    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function findDcsFromAllMantaInsts(ctx, next) {
                ...
                sapi.getApplicationObjects(ctx.app.uuid, {include_master: true}, function (err, objs) {
                    if (err) {
                        next(err);
                        return;
                    }
                    ...
                    if (instsSansDc.length > 0) {
                        let summary = instsSansDc.map(function (inst) {
                            return format('%s (%s)', inst.uuid, inst.params.alias);
                        });
                        next(new VError('%d manta instances do not have metadata.DATACENTER set:\n%s',
                            instsSansDc.length,
                            summary.join('\n    ')));
                    } else {
                        next();
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function theVms(ctx, next) {
                let uuids = ctx.insts.map(function onInst(inst) {
                    return inst.uuid
                }).join(',');
                ...
                vasync.forEachPipeline({
                    ...
                    func: function checkOneWebapi(webapi, nextWebapi) {
                                ...
                                switch (server) {
                                    ...
                                    default:
                                        ...
                                        webapi.err = new VError(
                                            ...
                                            server, webapi.uuid, webapi.alias);
                                        break;
                                }
                                nextWebapi();
                            });
                        ...
                        req.on('error', function onErr(err) {
                            webapi.isV2 = false;
                            webapi.err = new VError(err,
                                'could not "GET %s/" to check webapi instance %s (%s)',
                                webapi.adminIp, webapi.uuid, webapi.alias);
                            nextWebapi();
                        });
