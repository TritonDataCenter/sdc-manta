 ...
 * set is there.
 */
Migrator.prototype.ensureDelinkScripts = function ensureDelinkScripts(opts, cb) {
    ...
    scripts.storageNodeScripts = filenames
        .filter(f => /_stordelink.sh$/.test(f))
        .map(f => path.join(delinkDir, f));
    scripts.morayScripts = filenames
        .filter(f => /_moraydelink.sh$/.test(f))
        .map(f => path.join(delinkDir, f));
    if (scripts.morayScripts.length > 0) {
        let missingMorayScripts = [];
        opts.indexShards.forEach(function (shard) {
            var expected = path.join(delinkDir,
                `${opts.region}_${shard.name}_moraydelink.sh`);
            if (scripts.morayScripts.indexOf(expected) === -1) {
        ...
        if (missingMorayScripts.length > 0) {
            cb(new VError(
                ...
                'moray delink scripts: %s', missingMorayScripts.join(', ')));
        } else {
    ...
    for (let shardIdx = 0; shardIdx < opts.indexShards.length; shardIdx++) {
        ...
        try {
            records = zlib.gunzipSync(content)
                .toString('utf8')
                .trim()
                .split(/\n/g)
                .map(line => JSON.parse(line));
        } catch (parseErr) {
            cb(new VError(parseErr, 'could not load %s',
                shard.discoveryFile));
            return;
        }
        log.debug({discoveryFile: shard.discoveryFile, numRecords:
            records.length}, 'loaded discovery file');
        ...
        for (let i = 0; i < records.length; i++) {
            ...
            if (obj.size === 0) {
                ...
                for (let s = 0; s < obj.storageIds.length; s++) {
                    ...
                    // Add an `ln <old> <new>` command for each storage
                    // zone that has this object.
            ...
            if (obj.key.indexOf('"') !== -1 || obj.key.indexOf('\\') !== -1) {
                cb(new VError('object key cannot have `"` or `\\` characters: %s', obj.key));
                return;
            }

            if (!metadataCmdsFromShard.hasOwnProperty(shard.name)) {
                metadataCmdsFromShard[shard.name] = [];
            }
            ...
            metadataCmdsFromShard[shard.name].push(
                [
    ...
    shardNames = Object.keys(metadataCmdsFromShard);
    log.debug({
        numZeroByteObjs: numZeroByteObjs,
        numStorIds: storIds.length,
        numShards: shardNames.length
    ...
    for (let i = 0; i < storIds.length; i++) {
        ...
        let filename = path.join(delinkDir,
            `${opts.region}_${storId}_stordelink.sh`);
        try {
            fs.writeFileSync(
                filename,
                BASH_HEADER + cmds.join('\n') + '\n',
                {encoding: 'utf8'}
            );
        } catch (writeErr) {
            cb(writeErr);
        ...
        scripts.storageNodeScripts.push(filename);
        log.debug({filename: filename}, 'wrote stordelink script');
    }

    for (let i = 0; i < shardNames.length; i++) {
        let shardName = shardNames[i];
        let cmds = metadataCmdsFromShard[shardName];
        let filename = path.join(delinkDir,
            `${opts.region}_${shardName}_moraydelink.sh`);
        try {
            fs.writeFileSync(
                filename,
                BASH_HEADER + cmds.join('\n') + '\n',
        ...
        scripts.morayScripts.push(filename);
