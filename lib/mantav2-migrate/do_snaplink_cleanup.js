...
var promptYesNo = require('./clicommon').promptYesNo;
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function checkRequiredMetadatum(ctx, next) {
                ...
                // we are done.
                let required = ctx.mantaApp.metadata.SNAPLINK_CLEANUP_REQUIRED;
                ...
                switch (required) {
                    ...
                    default:
                        next(new VError(
                            ...
                            ctx.mantaApp, JSON.stringify(required)));
                        break;
                }
            },
            ...
            function getSetProgressMetadatum(ctx, next) {
                ...
                //          }
                //      }
                let progStr = ctx.mantaApp.metadata.SNAPLINK_CLEANUP_PROGRESS;
                if (!progStr) {
                    // Determine DCs in the region and set progress metadatum.
                    ui.info('Finding DCs in this region with Manta instances.');
                    ...
                    try {
                        ...
                        next(new VError(parseErr,
                            '"SNAPLINK_CLEANUP_PROGRESS" metadatum on ' +
                            '"manta" SAPI app (%s) is invalid JSON (%s).' +
                ...
                vasync.forEachPipeline({
                    ...
                    func: function doOne(task, nextTask) {
                        if (task.dc !== thisDcName) {
                            ...
                            ui.info('Determining if webapi instances in this DC are at V2.');
                            migrator.getWebapiV2Info(function onInfo(err, wInfo) {
                                let v1Insts = [];
                                let errInsts = [];
                                ...
                                if (v1Insts.length && v1Insts.length === wInfo.length) {
                                    msgs.append(format(
                                        'You must upgrade all %d webapi instances ' +
                                    ...
                                    let summary = v1Insts
                                        ...
                                        .join(', ');
                                    msgs.append(format(
                                        'You must upgrade the following %d webapi instance(s) ' +
                                        'in this DC (%s) to a recent enough V2 image ' +
                                        '(after 2019-12-09), and then re-run ' +
                                        '"mantav2-migrate snaplink-cleanup" to' +
                                ...
                                if (errInsts.length) {
                                    let summary = v1Insts
                                        .map(inst => format('%s (%s): %s', inst.uuid, inst.alias, inst.err))
                                        .join('\n    ');
                                    msgs.append(format(
                                ...
                                if (!v1Insts.length && !errInsts.length) {
                                    ctx.progress.infoFromDc[thisDcName].webapiAtV2 = true;
                                } else {
                                    ctx.progress.infoFromDc[thisDcName].webapiAtV2 = false;
                                }
                    ...
                    if (msgs.length) {
                        ui.info(
                            '\n' +
                            'The following must be resolved before snaplink-cleanup can proceed.\n' +
                            'Snaplinks cannot be fully cleaned until webapis are updated to\n' +
                            'a V2 image that no longer allows new snaplinks to be created.');
                        for (let msg of msgs) {
                            ui.info(bullet(msg));
                        }
                        next(new VError('webapi upgrades are required before snaplink-cleanup can proceed'));
                    } else {
            ...
            function checkDriverDc(ctx, next) {
                ...
                if (driverDc === thisDcName) {
                    ...
                    ui.info('Driver DC: %s (this one)', driverDc);
                    next();
                } else if (driverDc) {
                    ui.info('Driver DC: %s', driverDc);
                    next(new VError('continue with "mantav2-migrate snaplink-cleanup" in the driver DC %s', driverDc));
                } else if (!driverDc && Object.keys(ctx.progress.infoFromDc).length === 1) {
                    log.debug({driverDc: thisDcName},
                    ...
                    let msg = 'Would you like to use this DC as the driver? [y/N] ';
            ...
            function mkDbDir(ctx, next) {
                var skelDirs = [
                    DBDIR,
                    path.join(DBDIR, 'discovery')
                ];
                vasync.forEachPipeline({
                    inputs: skelDirs,
            ...
            // tool (snaplink-sherlock.sh) against every Manta index shard to
            // list all snaplinks. This tool needs to do a linear scan of the
            // full manta table, so can take a long time.
            //
            // Running "against a Manta index shard" specifically means
            // running the script on the compute node global zone holding the
            // postgres async VM for that shard (passing that VM uuid as an
            // argument). That will: create a manatee snapshot, create worker VM
            // using that snapshot, and run the table scan against a postgres
            // run in that work zone. The list of found snaplinks are written to
            // a file.
            //
            // Those files, one from each index shard, must be copied back
            // to /var/db/snaplink-cleanup dir on the driver DC headnode for
            // this phase to be considered complete.
            function discoveryPhase_Start(ctx, next) {
                ...
                for (let s of ctx.mantaApp.metadata.INDEX_MORAY_SHARDS) {
                    assert(s.host.endsWith(domainName),
                        `index shard "${s.host}" does not end with "${domainName}"`);
                    let shard = {
                        host: s.host,
                        name: s.host.slice(0, -domainName.length)
                    };
                    shard.discoveryFile = path.join(DBDIR, 'discovery',
                        `${region}_${shard.name}_snaplinks.tsv.gz`);
                    shard.discoveryFileExists = fs.existsSync(shard.discoveryFile);
                    shards.push(shard);
                    if (!shard.discoveryFileExists) {
                        ctx.missingShards.push(shard);
                    }
                }
                log.debug({shards: shards}, 'manta index shards info');

                if (ctx.missingShards.length === 0) {
                    ui.info('Phase 2: Have snaplink listings for all (%d) Manta index shards.', shards.length);
                    next();
                    return;
                }
                ...
                ui.info([
                    ...
                    'the async manatee for each Manta index shard. That will generate a',
                    `"${region}_{shard}_snaplinks.tsv.gz" file that must be copied back`,
                    `to "${DBDIR}/discovery/" on this headnode.`,
                    '',
                    'Repeat the following steps for each missing shard:',
                    '    https://github.com/joyent/manta/blob/master/docs/operator-guide/snaplink-removal.md#discovery',
                    '',
                    `Missing "*_snaplinks.tsv.gz" for the following shards (${ctx.missingShards.length} of ${shards.length}):`
                ].join('\n'));

                next();
            },

            function discoveryPhase_FindMissingShardAsyncs(ctx, next) {
                if (ctx.missingShards.length === 0) {
                    next();
                    return;
                }

                // TODO: gather and print some info on finding those asyncs
                // - Find a postgres zone (any shard) in this DC, else bail
                // - Run this to get the async for each shard:
                //    [root@headnode (staging-1) ~]# manta-oneach -J -z 9a90307c-439c-49cc-b2c4-1ccded301176 'manatee-adm pg-status -w -r async -s 2.moray.staging.joyent.us -H -o peername  2>/dev/null' | json result.stdout
                //    a5223321-600b-43eb-b66d-ebc0ab500046
                // - Figure out from all instances which DC that is in.
            ...
            // XXX START HERE
            function theNextThing(ctx, next) {
                ui.info('XXX next');
                XXX;
            },
        ]
...
do_snaplink_cleanup.help = [
    ...
    '',
    '1. Ensure all "webapi" instances are upgraded to mantav2-webapi image',
    '   that no longer allows new snaplink creation (images published after',
    '   2019-12-09).',
    '2. Discovery phase. Run a "snapshot-sherlock.sh" script against the',
    ...
    '   take a long time. The snaplink listing files are all gathered at',
    '   "/var/db/snaplink-cleanup/" on a designated driver DC in the region.',
    '3. Delink phase. The snaplink listing files from the previous phase are',
