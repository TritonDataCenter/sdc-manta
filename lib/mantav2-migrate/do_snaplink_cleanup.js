            ...
            funcs: [
                ...
                function getSetProgressMetadatum(ctx, next) {
                    ...
                    if (!progStr) {
                        ...
                        try {
                            ...
                            assert.object(ctx.progress.infoFromDc, 'infoFromDc');
                            assert.ok(Object.keys(ctx.progress.infoFromDc).length > 0, 'infoFromDc has entries');
                            assert.optionalString(ctx.progress.driverDc, 'driverDc');
                        } catch (validateErr) {
                            next(
                                new VError(
                                    validateErr,
                ...
                function checkDriverDc(ctx, next) {
                    ...
                    if (driverDc === thisDcName) {
                        ...
                        ui.info('Driver DC: %s (this one)', thisDcName);
                        saveProgress(ctx.progress, next);
                    } else {
                ...
                function discoveryPhase_Start(ctx, next) {
                    ...
                    ui.info(
                        [
                            ...
                            `"${region}_{shard}_sherlock.tsv.gz" file that must be copied back`,
                            ...
                            '',
                            `Missing "*_sherlock.tsv.gz" for the following shards (${ctx.missingShards.length} of ${shards.length}):`,
                        ].join('\n')
                    );
                    ...
                    next(new VError('sherlock files must be generated and ' +
                        'copied to "%s/discovery/" before snaplink cleanup ' +
                        'can proceed', DBDIR));
                },

                function XXXdiscoveryPhase_FindMissingShardAsyncs(ctx, next) {
                    if (ctx.missingShards.length === 0) {
                        next();
                        return;
                    }
