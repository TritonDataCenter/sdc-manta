                ...
                function discoveryPhase(ctx, next) {
                    ...
                    for (let s of ctx.mantaApp.metadata.INDEX_MORAY_SHARDS) {
                        ...
                        shard.discoveryFile = path.join(
                            DBDIR,
                            'discovery',
                            `${region}_${shard.name}_sherlock.tsv.gz`
                        );
                    ...
                    log.debug({indexShards: ctx.indexShards}, 'manta index shards info');
                    ...
                    migrator.ensureDelinkScripts(
                        {
                            ...
                            region: ctx.mantaApp.metadata.REGION
                        },
                        function (err, scripts, generated) {
                            ...
                            if (generated) {
                                ui.info('Created delink scripts in %s/delink.', DBDIR);
                            }
                            next();
                        }
                    );
                ...
                function confirmDelinkScriptsHaveBeenRun(ctx, next) {
                    ui.info(
                        [
                            ...
                            '',
                            '2. **Only after** these have all been run, copy each ',
                            '   "*_moraydelink.sh" script to a moray zone for the ',
                        ...
                        ctx.delinkScripts.storageNodeScripts.length,
                        DBDIR,
                        ctx.delinkScripts.morayScripts.length,
                        DBDIR
                    );

                    clicommon.promptConfirm({
                        confirmationStr: 'delinked',
                        msg: 'Enter "delinked" when all delink scripts have been successfully run: '
                    }, function (confirmed) {
                        if (confirmed) {
                            next();
                        } else {
                            ui.info('Aborting. Re-run this command when ' +
