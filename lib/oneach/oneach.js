 ...
 *         services, including webapi, moray, postgres, and several others.
 *         Operators work with these zones when they want to inspect or modify
 *         most Manta components.
 *
 ...
 *         inspect or modify the operating system itself.
 *
 *     The file transfer mechanism used by this tool implicitly trusts the
 *     contents of the zone's filesystem, so it's not secure if untrusted users
...
function mzCommandExecutor(args) {
    ...
    if (args.sdcMantaConfigFile !== null) {
        assertplus.string(args.sdcMantaConfigFile, 'args.sdcMantaConfigFile');
     ...
     */
    assertplus.ok(
        args.execMode === MZ_EM_COMMAND ||
            args.execMode === MZ_EM_SENDTOREMOTE ||
            args.execMode === MZ_EM_RECEIVEFROMREMOTE,
        'args.execMode'
    );
    ...
    if (this.ce_exec_mode === MZ_EM_COMMAND) {
        ...
        assertplus.ok(
            args.execClobber === null,
            'args.execClobber must not be specified for commands'
        );
        this.ce_exec_clobber = null;
    } else {
        assertplus.ok(
            args.execCommand === null,
            'args.execCommand must not be specified for file transfers'
        );
        ...
        this.ce_exec_clobber = !!args.execClobber;
    ...
    assertplus.object(args.log, 'args.log');
...
mzCommandExecutor.prototype._read = function() {
     ...
     */
    if (
        this.ce_exec_mode === MZ_EM_COMMAND &&
    ...
    if (this.ce_scope_services !== null) {
        for (i = 0; i < this.ce_scope_services.length; i++) {
            if (!services.serviceNameIsValid(this.ce_scope_services[i])) {
                setImmediate(
                    this.emit.bind(this),
                    'error',
                    new VError(
     ...
     */
    if (
        (this.ce_exec_mode === MZ_EM_SENDTOREMOTE ||
...
mzCommandExecutor.prototype.stageSetupManta = function(_, callback) {
    ...
    this.ce_manta.loadSdcConfig(function(err) {
        ...
        self.ce_manta.fetchDeployed(function(err2) {
            if (err2) {
                callback(new VError(err2, 'manta-adm fetch deployed'));
                return;
            }
...
mzCommandExecutor.prototype.stageSetupBindIp = function(_, callback) {
    ...
    args = {};
    zonename = child_process
        .execSync('/usr/bin/zonename', {encoding: 'utf8'})
        .trimRight();
    if (zonename !== 'global') {
    ...
    forkexec.forkExecWait(
        ...
        function(err, result) {
            ...
            try {
                ...
                callback(
                    new VError(ex, 'failed to select local IP: parsing sysinfo')
                );
                return;
            }
...
mzCommandExecutor.prototype.stageIdentifyScope = function(_, callback) {
    ...
    err = this.ce_manta.eachZoneByFilter(
        ...
        function(zoneinfo) {
            ...
            if (!self.ce_servers.hasOwnProperty(zoneinfo['SERVER_UUID'])) {
            ...
            self.ce_servers[zoneinfo['SERVER_UUID']].s_cmds.push({
                ...
                cmd_command:
                    self.ce_exec_mode === MZ_EM_COMMAND
                        ? self.makeUrScript(zoneinfo['ZONENAME'])
                        : null,
                cmd_result: null
            });
    ...
    if (this.ce_scope_globals) {
        jsprim.forEachKey(this.ce_servers, function(s) {
            if (self.ce_exec_mode === MZ_EM_COMMAND) {
                fprintf(self.ce_stream, 'host %s: %s', s, self.ce_exec_command);
            } else {
                fprintf(self.ce_stream, 'host %s: file transfer', s);
            }
        });
    } else {
...
mzCommandExecutor.prototype.stageExecuteCommands = function(_, callback) {
    ...
    if (this.ce_scope_globals) {
        ...
        jsprim.forEachKey(this.ce_servers, function(_s, server) {
...
mzCommandExecutor.prototype.queueExecuteOneTransfer = function(cmd, qcallback) {
    ...
    if (this.ce_exec_mode === MZ_EM_SENDTOREMOTE) {
        ...
        if (this.ce_scope_globals) {
            ...
            urargs['dst_dir'] = path.join(
                '/zones',
                cmd.cmd_zonename,
                'root',
                this.ce_exec_dir
            );
            patherr = this.validateZonePath(
                urargs['dst_dir'],
                cmd.cmd_zonename
            );
        }
        ...
        if (this.ce_scope_globals) {
            ...
            urargs['dst_file'] = path.join(this.ce_exec_dir, cmd.cmd_zonename);
            urargs['src_file'] = path.join(
                '/zones',
                cmd.cmd_zonename,
                'root',
                this.ce_exec_file
 ...
 */
mzCommandExecutor.prototype.urDone = function(
    label,
    cmd,
    summary,
    err,
        ...
        this.rt_results
            .sort(function(r1, r2) {
                ...
                if (r1.hasOwnProperty('service')) {
                    ...
                    r = r1['zonename'].localeCompare(r2['zonename']);
                    if (r !== 0) {
                        return r;
                    }
                }
...
function mzValidateScopeParameters(args) {
    ...
    assertplus.optionalArrayOfString(args.scopeServices, 'args.scopeServices');
    ...
    havefilters =
        args.scopeComputeNodes !== null ||
        args.scopeZones !== null ||
        args.scopeServices !== null;

    if (args.scopeAllZones && havefilters) {
        return new VError(
            'cannot specify zones, services, ' +
    ...
    if (!args.scopeAllZones && !havefilters) {
        return new VError(
            'must explicitly request all zones to operate on all zones'
        );
    }
