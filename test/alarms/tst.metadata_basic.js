...
function generateTestCases() {
    ...
    testCases.push({
        ...
        verify: function verifyEmpty(md) {
            md.eachEvent(function(eventName) {
                throw new VError(
                    'empty metadata found an event: "%s"',
                    eventName
                );
            });
    ...
    testCases.push({
        ...
        verify: function verifyValid(md) {
            ...
            assertplus.equal(pt.pt_checks[0].ptc_type, sampleChecks[0].type);
            assertplus.deepEqual(
                pt.pt_checks[0].ptc_config,
             ...
             * subsystem of sdc-manta.
             */
            pgname = alarm_metadata.probeGroupNameForTemplate(pt, sampleEvent);
            ...
            parsed = alarm_metadata.testingParseProbeGroupName('mygroup;v=2');
            assertplus.object(parsed.error, 'error');
            assertplus.ok(parsed.error instanceof Error);
            assertplus.ok(
                /unrecognized version "2" in probe group/.test(
            ...
            assertplus.strictEqual(false, md.probeGroupIsRemovable('foobar'));
            /* a legacy probe group name */
            assertplus.strictEqual(null, md.probeGroupEventName('ops-alert'));
            assertplus.strictEqual(true, md.probeGroupIsRemovable('ops-alert'));
            /* an operator-created probe group name */
    ...
    ['event', 'scope', 'checks', 'ka'].forEach(function(field) {
        ...
        testCases.push({
            name: sprintf(
                'schema mismatch: missing required field: "%s"',
                field
            ),
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^parse "input": property "0.legacyName": ' +
                'object.*found,.*string.*required'
        )
    });
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^parse "input": property "0.scope.service":' +
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^parse "input": property "0.scope.global": ' +
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^input: probe 1: field ka.response: ' +
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^input: probe 2: re-uses event name ' +
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^input: probe 1: field "event": must begin ' +
                'with "upset.manta."$'
        )
    });
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^template "input: probe 1": templates ' +
    ...
    testCases.push({
        ...
        errmsg: new RegExp(
            '^template "input: probe 1": unknown ' +
...
function runTestCase(testcase) {
    ...
    if (testcase.hasOwnProperty('errmsg')) {
        assertplus.ok(
            testcase.errmsg instanceof RegExp,
