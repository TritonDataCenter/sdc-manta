...
lineReader
    .on('line', function _onLine(rawLine) {
        var idx;
        ...
        if (obj.key.indexOf('"') !== -1 || obj.key.indexOf('\\') !== -1) {
            throw new Error(
                'key must not have " or \\ characters. Got: ' + obj.key
            );
        }
    ...
    .on('close', function() {
        var dir =
            shardId + '.' + new Date().toISOString().replace(/[-:.]/g, '');
        var filename;
        var idx;
        var storageId;
        var storageIds;
        ...
        fs.mkdir(dir, function _onMkdir(err) {
            ...
            fs.writeFileSync(
                filename,
                BASH_HEADER + metadataCommands.join('\n') + '\n',
                {encoding: 'utf8'}
            );
            ...
            for (idx = 0; idx < storageIds.length; idx++) {
                ...
                fs.writeFileSync(
                    filename,
                    BASH_HEADER + storageCommands[storageId].join('\n') + '\n',
                    {encoding: 'utf8'}
                );
            }
            ...
            console.error(
                util.format(
                    'Lines: %d, mdata Updates: %d',
                    numLines,
                    metadataCommands.length
                )
            );
        });
    });
